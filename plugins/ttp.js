...
let handler = async (m, { conn, text, command }) => {
  ...
  if (/2$/i.test(command)) {
    let url = await fetch(
      API("https://salism3api.pythonanywhere.com", "/text2img/", { text: teks })
    );
  ...
  if (/3$/i.test(command)) {
    let stiker = await sticker5(
      null,
      API("hardianto", "/api/maker/ttp", { text: teks }, "apikey"),
      packname,
      author
    );
    if (stiker) return await conn.sendFile(m.chat, stiker, "", "", m);
  ...
  if (/4$/i.test(command)) {
    let url = await fetch(
      API("https://salism3api.pythonanywhere.com", "/text2img/", {
        text: teks,
