...
let handler = async (m, { conn, usedPrefix, command }) => {
  ...
  try {
    ...
    if (!/image/.test(mime))
      return m.reply(`balas gambar dengan caption *.sfire*`);
    let img = await q.download();
    img = `https://api.zeks.me/api/burning-image?apikey=administrator&image=${await up(
      img
    )}`;
    m.reply(`tunggu...`);
...
handler.help = ["sfire", "stickerfire", "stikerfire"].map(
  (v) => v + " [reply/kirim gambar]"
);
